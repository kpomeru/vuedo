	...
	>
		<h5 class="flex flex-col space-y-1 items-start">
			...
			<span class="flex items-center space-x-1">
				<Badge
					class="slate"
					:title="`Created ${project.createdAt.date}`"
				>
					<svg
				...
				<Badge
					class="dark"
					:title="`${project.comments.length} comments`"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 20 20"
				...
				<Badge :title="`${project.tasks.length} tasks`">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						viewBox="0 0 20 20"
						fill="currentColor"
		...
		<div class="flex items-center space-x-3 text-xs">
			...
			<template v-if="project.title.toLowerCase() !== 'general'">
				<Dropdown align-to="right">
					<template #trigger>
						<span class="isButton" title="Delete project">
